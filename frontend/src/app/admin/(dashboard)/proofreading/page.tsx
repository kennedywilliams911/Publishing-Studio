...
export const metadata = { title: "Proofreading" };
export const dynamic = "force-dynamic";

export default async function ProofreadingPage({
  searchParams,
}: {
  ...
  const { items, page, totalPages, total } = await queryArticles({
    q: sp.q,
    proofreadingStatus:
      sp.proofreadingStatus === "not_started" ||
      sp.proofreadingStatus === "requested" ||
      sp.proofreadingStatus === "in_progress" ||
      sp.proofreadingStatus === "approved"
        ? sp.proofreadingStatus
        : undefined,
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-ink-500 dark:text-parchment-300">
          {total} article{total === 1 ? "" : "s"} currently in your review
          queue.
        </p>
      </div>

      <ArticleFilters showStatusFilter={false} showProofreadingFilter />

      {items.length === 0 ? (
        <EmptyState
          title="No proofreading items yet."
          description="Mark an article as requested, in progress, or approved to see it here."
        />
      ) : (
